...
from company.models import Filial, Otdel, Sotr
...
filial_ids = [1, 2]
query_3 = Sotr.objects.filter(Q(otdel__filial_id=filial_ids[0]) | Q(otdel__filial_id=filial_ids[1])).values_list('full_name', flat=True)
...
query_4 = Sotr.objects.filter(otdel__filial_id__in=filial_ids).values_list('full_name', flat=True)

# 5. Получи список ФИО сотрудников, у которых не указан email
query_5 = Sotr.objects.filter(email__isnull=True).values_list('full_name', flat=True).values_list('full_name', flat=True)
